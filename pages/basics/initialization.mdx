...
`bard-ai` v2 comes with the ability to use cookies from different Google users. These will take the form of a `__Secure-#PSID` where `X` is a different number than 1. If you wish to use such a cookie, simply prefix your `COOKIE` with `__Secure-#PSID=` (with your account number).

For example, if you use Bard manually and the URL ends with `/u/3`, you would copy the cookie that starts with `__Secure-3PSID`.
